...
def batch_analyze_quality(texts: List[str], batch_size: int = 500, max_workers: Optional[int] = None) -> List[Dict[str, float]]:
    """배치 단위 고성능 품질 분석 (병렬 처리 최적화)"""
    if max_workers is None:
        max_workers = min(32, multiprocessing.cpu_count() * 2)  # CPU 코어 * 2

    results = []

    # ThreadPoolExecutor로 I/O 병목 최소화
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        # 전체를 한 번에 제출 (대기 시간 최소화)
        futures = []
        for i in range(0, len(texts), batch_size):
            batch_texts = texts[i:i + batch_size]
            for text in batch_texts:
                futures.append(executor.submit(analyze_coref_quality_cached, text))

        # 완료되는 대로 수집 (tqdm 진행률)
        for future in tqdm(as_completed(futures), total=len(futures), desc="  ⚡ 품질 분석", unit="샘플", leave=False):
            try:
...
def preprocess_domain_texts(dataset, source: DatasetSource, tokenizer, target_seq_len: int, is_iterator: bool = False) -> List[str]:
    ...
    for item in iterator:
        ...
        try:
            ...
            # 최소 길이 체크
            if token_len < target_seq_len * 0.3:
                skipped += 1
                continue
    ...
    for seq_len in seq_lengths:
        ...
        for source in sources:
            try:
                ...
                # 샘플링하여 분석 (메모리 효율성)
                sample_size = min(2000, len(dataset))  # 샘플 크기 증가
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max-samples", type=int, default=None, help="최대 샘플 수 (None=무제한)")
