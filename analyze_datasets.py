...
def analyze_dataset(dataset_name, subset=None, split='train', text_field='text', sample_size=1000):
    """데이터셋 분석"""
    print(f"\n{'='*80}")
    ...
    try:
        ...
        # 개체 반복 패턴 분석
        print(f"\n[개체 반복 패턴 분석] (샘플 {min(len(texts), 500)}개)")
        repeated_count = 0
        all_repeated_entities = []
        pronoun_counts = []

        for text in tqdm(texts[:500], desc="분석 중"):
        ...
        repeat_ratio = (repeated_count / min(len(texts), 500)) * 100
